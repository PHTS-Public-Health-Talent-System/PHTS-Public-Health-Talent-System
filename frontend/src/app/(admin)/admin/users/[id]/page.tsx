...
const getRolePolicyHint = (roleCode: string) => {
  if (roleCode === 'HEAD_SCOPE') {
    return 'สิทธิ์นี้อิงข้อมูลตำแหน่งพิเศษจาก HRMS และอาจถูกซิงก์ปรับอัตโนมัติ';
  }
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8 max-w-[1400px] mx-auto">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <Card className="border-border shadow-sm">
          <CardContent className="p-4">
            <p className="text-xs text-muted-foreground">ขอบเขตการดูแล</p>
            <p className="mt-1 text-lg font-semibold">{user.scopes?.length ?? 0} รายการ</p>
            <p className="text-xs text-muted-foreground">
              {user.scopes?.length ? 'คุณมีขอบเขตการดูแลที่ต้องรับผิดชอบ' : 'ใช้สิทธิ์พื้นฐานของบทบาท'}
            </p>
          </CardContent>
        </Card>
        <Card className="border-border shadow-sm">
                ...
                <div className="space-y-2 sm:col-span-2 pt-2 border-t border-border/50">
                  ...
                  {!user.scopes || user.scopes.length === 0 ? (
                    <p className="text-sm text-muted-foreground/80 italic">
                      ไม่พบขอบเขตการดูแลเพิ่มเติม ใช้สิทธิ์ตามบทบาทที่กำหนดไว้
                    </p>
                  ) : (
