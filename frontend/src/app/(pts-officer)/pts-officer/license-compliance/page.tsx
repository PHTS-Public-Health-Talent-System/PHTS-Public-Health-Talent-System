...
import {
  ...
  useNotifyLicenseCompliance,
} from '@/features/license-compliance/hooks';
import type { LicenseComplianceListItem } from '@/features/license-compliance/api';
...
interface LicenseComplianceItem {
  id: string;
  citizenId: string;
  name: string;
  position: string;
...
function deriveStatus(bucket: AlertBucket, daysLeft: number): LicenseComplianceItem['status'] {
...
function getStatusBadge(status: LicenseComplianceItem['status']) {
  switch (status) {
    case 'expired':
      return (
...
function mapAlertRow(row: LicenseComplianceListItem): LicenseComplianceItem {
...
export default function LicenseCompliancePage() {
  ...
  const { data: summaryData } = useLicenseComplianceSummary();
  const { data: expiredData } = useLicenseComplianceList({ bucket: 'expired' });
  const { data: expiring30Data } = useLicenseComplianceList({ bucket: '30' });
  const { data: expiring60Data } = useLicenseComplianceList({ bucket: '60' });
  const { data: expiring90Data } = useLicenseComplianceList({ bucket: '90' });
  const notifyMutation = useNotifyLicenseCompliance();

  const alerts = useMemo<LicenseComplianceItem[]>(() => {
    const normalize = (data: unknown) =>
      Array.isArray(data) ? data.map((row) => mapAlertRow(row as LicenseComplianceListItem)) : [];
    ...
    const byCitizen = new Map<string, LicenseComplianceItem>();
  ...
  const notifyByRows = async (rows: LicenseComplianceItem[]) => {
  ...
  const handleNotifyOne = async (alert: LicenseComplianceItem) => {
