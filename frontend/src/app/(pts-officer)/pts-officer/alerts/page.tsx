...
function deriveStatus(bucket: LicenseAlert["bucket"], daysLeft: number): LicenseAlert["status"] {
...
export default function AlertsPage() {
  ...
  const alerts = useMemo<LicenseAlert[]>(() => {
    const normalize = (data: unknown, bucket: LicenseAlert["bucket"]) => {
      ...
      return data.map((row) => {
        ...
        const id = `${entry.citizen_id ?? "unknown"}-${bucket}`
        const daysLeft = typeof entry.days_left === "number" ? entry.days_left : 0
        const notifiedDate = notifiedMap[id]
        return {
          id,
          name: entry.full_name ?? "-",
          position: entry.position_name ?? "-",
          department: "-",
          profession: entry.position_name ?? "-",
          licenseNumber: "-",
          licenseExpiry: formatThaiDate(entry.license_expiry ?? null),
          daysLeft,
          status: deriveStatus(bucket, daysLeft),
          notified: Boolean(notifiedDate),
          notifiedDate,
          bucket,
        }
      })
    ...
    return [
      ...normalize(expiredData, "expired"),
      ...normalize(expiring30Data, "30"),
      ...normalize(expiring60Data, "60"),
      ...normalize(expiring90Data, "90"),
    ]
  }, [expiredData, expiring30Data, expiring60Data, expiring90Data, notifiedMap])

  const filteredAlerts = alerts.filter((alert) => {
    const matchesSearch = alert.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      alert.position.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  const handleNotifyOne = (id: string) => {
    const today = new Date().toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: 'numeric' })
    setNotifiedMap((prev) => ({ ...prev, [id]: today }))
  }
    ...
    <div className="p-8">
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {selectedAlerts.length > 0 && (
            <Button className="gap-2" onClick={handleNotify}>
              <Mail className="h-4 w-4" />
              ส่งอีเมลแจ้งเตือน ({selectedAlerts.length})
            </Button>
          )}
        </div>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-secondary/30 hover:bg-secondary/30">
                  ...
                  <TableHead className="font-semibold text-center">แจ้งเตือนแล้ว</TableHead>
                    ...
                    <TableCell className="text-center">
                      {!alert.notified && (
                        <Button 
                          ...
                          size="sm" 
                          className="gap-1"
                          onClick={() => handleNotifyOne(alert.id)}
                        >
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
                <li>
                  - ผู้ที่ใบอนุญาตหมดอายุจะ<span className="text-destructive font-medium">ไม่สามารถรับเงิน พ.ต.ส.</span>ได้จนกว่าจะต่ออายุใบอนุญาต
                </li>
                <li>
                  - ควรแจ้งเตือนล่วงหน้าอย่างน้อย 30-90 วันก่อนใบอนุญาตหมดอายุ
                </li>
                <li>
                  - สามารถเลือกหลายรายการแล้วส่งอีเมลแจ้งเตือนพร้อมกันได้
                </li>
              </ul>
            </div>
          </div>
      ...
      <Dialog open={showNotifyDialog} onOpenChange={setShowNotifyDialog}>
        <DialogContent className="bg-card border-border">
          <DialogHeader>
            <DialogTitle>ส่งอีเมลแจ้งเตือน</DialogTitle>
            <DialogDescription>
              คุณต้องการส่งอีเมลแจ้งเตือนไปยังบุคลากรที่เลือก {selectedAlerts.length} คน ใช่หรือไม่?
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            <ul className="space-y-1 max-h-[200px] overflow-auto">
              {alerts.filter(a => selectedAlerts.includes(a.id)).map(a => (
      ...
      <Dialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
        <DialogContent className="bg-card border-border">
          <DialogHeader>
            ...
            <DialogTitle className="text-center">ส่งอีเมลสำเร็จ</DialogTitle>
            <DialogDescription className="text-center">
              ส่งอีเมลแจ้งเตือนเรียบร้อยแล้ว
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
