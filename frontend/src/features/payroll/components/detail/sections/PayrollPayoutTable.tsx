      ...
      <Table>
        <TableHeader className="sticky top-0 z-10 bg-muted/50 shadow-sm">
          <TableRow>
            ...
            <TableHead className="w-[120px] text-right font-bold text-foreground">สุทธิ</TableHead>
              ...
              return (
                <TableRow key={person.id} className="group transition-colors hover:bg-muted/30">
                  ...
                  <TableCell className="sticky left-0 z-10 bg-background shadow-[1px_0_0_0_rgba(0,0,0,0.05)] transition-colors group-hover:bg-muted/30">
                    <div className="flex cursor-pointer flex-col" onClick={() => onOpenAllowanceDetail(person)}>
                      <span className="text-sm font-medium text-foreground transition-colors group-hover:text-primary">
                        {person.name}
                      </span>
                      <span className="text-[11px] font-mono text-muted-foreground">{person.citizenId}</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-center">
                    <Badge variant="outline" className="h-5 bg-background px-1.5 text-[10px] font-normal">
                  ...
                  <TableCell className="text-right text-sm tabular-nums">
                    ...
                    >
                      {person.retroactiveAmount !== 0 && formatThaiNumber(person.retroactiveAmount)}
                    </span>
                  </TableCell>

                  <TableCell className="text-right text-sm tabular-nums">
                    {person.deductionAmount > 0 ? (
                      <span className="font-medium text-orange-700">-{formatThaiNumber(person.deductionAmount)}</span>
                    ) : (
                  ...
                  <TableCell className="text-right tabular-nums">
                    <span
                      className={cn(
                        ...
                        person.totalAmount > 0 ? 'bg-emerald-50 text-emerald-700' : 'text-muted-foreground',
                      )}
                    >
