...
export function useUpdateAccessReviewItem() {
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["access-review-cycle"] });
      await queryClient.invalidateQueries({ queryKey: ["access-review-cycles"] });
    },
  });
}
...
export function useResolveAccessReviewQueueItems() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({
      payload,
    }: {
      payload: { queue_ids: number[]; action: "RESOLVE" | "DISMISS"; note?: string };
    }) => resolveAccessReviewQueueItems(payload),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["access-review-queue"] });
      await queryClient.invalidateQueries({ queryKey: ["system-jobs"] });
    },
