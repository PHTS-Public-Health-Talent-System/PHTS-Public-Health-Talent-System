...
export type LeaveQuotaStatusItem = {
  leave_type: string;
  type_name: string;
  rule_type: "cumulative" | "per_event";
  tracks_balance: boolean;
  limit: number | null;
  reference_fiscal_year: number | null;
  reference_limit: number | null;
  used_as_of_today: number;
  remaining_as_of_today: number | null;
  used_after_leave: number;
  remaining_after_leave: number | null;
  over_quota_after_leave: boolean;
  exceed_date_after_leave: string | null;
  has_quota_data: boolean;
};

export type LeaveQuotaStatusResponse = {
  leave_id: number;
  fiscal_year: number;
  as_of_date: string;
  current_leave: {
    ...
    type_name: string;
    duration: number;
    leave_status: "upcoming" | "active" | "completed";
    rule_type: "cumulative" | "per_event";
    tracks_balance: boolean;
    limit: number | null;
    reference_fiscal_year: number | null;
...
export async function getLeaveQuotaStatus(
  ...
  const res = await api.get<ApiResponse<LeaveQuotaStatusResponse>>(
    `${LEAVE_BASE}/${leaveRecordId}/quota-status`,
  );
  return res.data.data;
}
