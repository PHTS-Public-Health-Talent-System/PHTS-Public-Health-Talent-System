...
export function useUpsertLeaveRecordExtension() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["payroll-period-leaves"] });
      qc.invalidateQueries({ queryKey: ["payroll-period-leave-professions"] });
...
export function useLeaveQuotaStatus(
  leaveRecordId: number | string | null,
) {
  return useQuery<LeaveQuotaStatusResponse>({
    queryKey: ["leave-record-quota-status", leaveRecordId],
    queryFn: () => getLeaveQuotaStatus(leaveRecordId!),
    enabled: Boolean(leaveRecordId),
  });
}
