...
export const getOnBehalfMetadata = (submission: Record<string, unknown> | null | undefined) => {
  const createdByOfficerId = Number(submission?.created_by_officer_id ?? 0)
  const createdByOfficerRole =
    typeof submission?.created_by_officer_role === "string"
      ? submission.created_by_officer_role.trim() || null
      : null
  const createdMode =
    typeof submission?.created_mode === "string"
      ? submission.created_mode.trim()
      : ""

  return {
    createdByOfficerId:
      Number.isInteger(createdByOfficerId) && createdByOfficerId > 0
        ? createdByOfficerId
        : null,
    createdByOfficerRole,
    isOfficerCreated:
      createdMode === "OFFICER_ON_BEHALF" ||
      (createdByOfficerRole === "PTS_OFFICER" &&
        Number.isInteger(createdByOfficerId) &&
