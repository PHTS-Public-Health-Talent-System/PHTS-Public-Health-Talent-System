...
export function Step5Review({
  ...
  const signatureOk =
    !requiresSignature ||
    (data.signatureMode === 'SAVED' ? hasSavedSignature : !!data.signature);
  if (requiresSignature && !signatureOk) missing.push({ label: 'ลายเซ็น', step: 5 });

  useEffect(() => {
    if (!requiresSignature) return;
    if (data.signatureMode) return;
    updateData('signatureMode', hasSavedSignature ? 'SAVED' : 'NEW');
  }, [hasSavedSignature, data.signatureMode, updateData, requiresSignature]);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-1">
        <h3 className="text-xl font-semibold text-primary">
          {requiresSignature ? 'ตรวจสอบความถูกต้องและลงนาม' : 'ตรวจสอบความถูกต้อง'}
        </h3>
            ...
            {requiresSignature ? (
              ...
              <Alert className="bg-primary/5 border-primary/20 text-foreground">
                ...
                <AlertDescription className="text-sm leading-relaxed">
                  คำขอนี้เป็นคำขอที่เจ้าหน้าที่ พ.ต.ส. บันทึกแทนบุคลากร
                  ระบบจึงไม่บังคับให้ลงลายมือชื่อก่อนบันทึกหรือส่งคำขอ
                </AlertDescription>
              </Alert>
            )}
