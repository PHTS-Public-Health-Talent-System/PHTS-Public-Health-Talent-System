...
export const resolveRateMappingDisplay = (
  ...
  const criteriaByItem = isNoneItem
  ...
  const criteriaByRateId = mapping.rateId
    ? group?.criteria.find((item) =>
      item.rateId === mapping.rateId || item.subCriteria?.some((subItem) => subItem.rateId === mapping.rateId),
    )
    : undefined;
  const criteria = criteriaByItem ?? criteriaByRateId;
  const subCriteriaByItem = mapping.subItemId
    ? criteria?.subCriteria?.find((item) => item.id === mapping.subItemId)
    : undefined;
  const subCriteriaByRateId = mapping.rateId
    ? criteria?.subCriteria?.find((item) => item.rateId === mapping.rateId)
    : undefined;
  const subCriteria = subCriteriaByItem ?? subCriteriaByRateId;
