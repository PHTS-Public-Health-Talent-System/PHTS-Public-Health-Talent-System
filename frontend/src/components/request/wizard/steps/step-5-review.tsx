...
export function Step5Review({ data, updateData, onGoToStep, prefillOriginal }: Step5Props) {
  ...
  const employeeTypeLabel = data.employeeType
    ? PERSONNEL_TYPE_LABELS[data.employeeType]
    : "-"
          ...
          <section className="space-y-3">
            ...
            <div className="bg-muted/30 p-4 rounded-lg text-sm space-y-2 border">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{employeeTypeLabel}</span>
              </div>
          ...
          <div className="space-y-3">
            ...
            >
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="SAVED" disabled={!hasSavedSignature}>
                  ใช้ลายเซ็นที่มี
                </TabsTrigger>
                <TabsTrigger value="NEW">วาดลายเซ็นใหม่</TabsTrigger>
              </TabsList>

              <TabsContent value="SAVED" className="mt-4">
                <Card className="border">
                  <CardContent className="p-4 space-y-3">
                    <Label className="text-sm text-muted-foreground">ลายเซ็นที่มีอยู่ในระบบ</Label>
                    {isSignatureLoading ? (
                      <div className="text-sm text-muted-foreground">กำลังโหลดลายเซ็น...</div>
                    ) : signature?.data_url ? (
                      <div className="flex items-center justify-center rounded-lg border bg-muted/10 p-4">
                        <Image
                          src={signature.data_url}
                          alt="ลายเซ็นที่มีในระบบ"
                          width={320}
                          height={160}
                      ...
                      <div className="text-sm text-muted-foreground">
                        ยังไม่มีลายเซ็นในระบบ กรุณาวาดลายเซ็นใหม่
                      </div>
                    )}
                  </CardContent>
                </Card>
