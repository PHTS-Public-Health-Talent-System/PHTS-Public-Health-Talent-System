...
import { runLocalPaddle } from '@/modules/ocr/services/ocr-local-paddle.service.js';
...
const outputPath =
  ...
  path.join(repoRoot, 'ocr', 'output_text', 'OCR_paddle_local_tuned.txt');
...
const run = async (): Promise<void> => {
  ...
  for (const fileName of fileNames) {
    ...
    const result = await runLocalPaddle(fileName, buffer);
  ...
  console.log(`Paddle OCR benchmark finished: ${stats.length} files`);
...
run().catch((error) => {
  ...
  console.error(`[ocr-paddle-benchmark] ${message}`);
  process.exitCode = 1;
});
