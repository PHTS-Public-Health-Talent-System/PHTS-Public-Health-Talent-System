/**
 * support module - request orchestration
 *
 */
...
export const listMyTickets = catchAsync(
  async (req: Request, res: Response<ApiResponse>) => {
    if (!req.user) throw new AuthenticationError("Unauthorized access");
...
export const createMessage = catchAsync(
  async (req: Request, res: Response<ApiResponse>) => {
    ...
    if (!isOwner && !isAdmin) {
      throw new ValidationError(
        "You do not have permission to post on this ticket",
      );
    }
    if (
      !isAdmin &&
    ...
    if (!normalizedMessage && attachments.length === 0) {
      throw new ValidationError("Please add a message or attachment");
    }
